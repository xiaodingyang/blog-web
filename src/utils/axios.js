...
axios.defaults.baseURL = process.env.NODE_ENV !== 'development' ? 'http://xiaodingyang.com:8000/api' : 'http://localhost:8001/api';
// request拦截器
axios.interceptors.request.use(
    config => {
        // store.dispatch(other.setLoading(true))
        return config
    }
);
    ...
    response => {
        ...
        */
        ...
        if (response.status === 200) {
            if (userInfo && path === '/login') window.location.href = '/index'
